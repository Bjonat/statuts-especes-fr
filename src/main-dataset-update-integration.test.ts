...
describe('PWA dataset update wiring', () => {
  ...
  it('does not give Workbox runtime caches for dataset files', () => {
    ...
    expect(viteSource).toMatch(/data\/manifest\.json/)
    expect(viteSource).toMatch(/data\/taxa-\*\.json/)
    expect(viteSource).toMatch(/data\/status-definitions-\*\.json/)
    expect(viteSource).toMatch(/data\/status-links-\*\.json/)
    ...
    expect(packageSource).not.toMatch(/playwright/)
